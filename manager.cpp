...
{
    OutputDebugStringA("[Manager] TransitionToBossStage - ?J?n\n");
    m_IsBossStage = true;

    // ?v???C???[??��
    Player* player = GetGameObject<Player>();
    if (player) {
        OutputDebugStringA("[Manager] ?v???C???[??u????Z?b?g\n");
        player->SetPosition(XMFLOAT3(0.0f, -0.5f, 0.0f));
    }

    // ?v???C???[?A?t?B?[???h??O???????I?u?W?F?N?g??j???i???X?g??????j
    OutputDebugStringA("[Manager] ?I?u?W?F?N?g?j????????J?n?????...\n");
    ...
    std::string destroyMsg = "[Manager] ?I?u?W?F?N?g?j??????????????? (???: " + std::to_string(destroyedCount) + ")\n";
    OutputDebugStringA(destroyMsg.c_str());

    // ?{?X??????????
    OutputDebugStringA("[Manager] ?{?X???????????????...\n");
    ...
    OutputDebugStringA("[Manager] ?????????\n");

    // ?{?X?G?l?~?[?????
    OutputDebugStringA("[Manager] ?{?X?G?l?~?[???????J?n?????...\n");
    BossEnemy* boss = AddGameObject<BossEnemy>();
    boss->SetPosition(XMFLOAT3(0.0f, 1.5f, 10.0f)); // ?v???C???[??????O????z?u
    OutputDebugStringA("[Manager] ?{?X?G?l?~?[????????\n");

    // ?{?X?????p??A?C?e???????i?{?X????????????? Z=-18.0f ?t???z?u?j
    OutputDebugStringA("[Manager] ?{?X?????p??A?C?e???????????...\n");
    ...
    OutputDebugStringA("[Manager] TransitionToBossStage - ????I??\n");
}
