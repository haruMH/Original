...
void Renderer::Init() {
  ...
  if (SUCCEEDED(hr) && renderTarget != nullptr) {
  ...
  // デプスステンシルビュー作成
  if (SUCCEEDED(hr) && depthStencile != nullptr) {
    D3D11_DEPTH_STENCIL_VIEW_DESC depthStencilViewDesc{};
    depthStencilViewDesc.Format = textureDesc.Format;
    depthStencilViewDesc.ViewDimension = D3D11_DSV_DIMENSION_TEXTURE2D;
    depthStencilViewDesc.Flags = 0;

    m_Device->CreateDepthStencilView(depthStencile, &depthStencilViewDesc,
                                     &m_DepthStencilView);
    depthStencile->Release();
  }

  // ビューが両方無事に作れている場合のみセット
  ...
  m_Device->CreateDepthStencilState(&depthStencilDesc,
                                    &m_DepthStateEnable); // 深度有効ステート

  // depthStencilDesc.DepthEnable = FALSE;
  depthStencilDesc.DepthWriteMask = D3D11_DEPTH_WRITE_MASK_ZERO;
  m_Device->CreateDepthStencilState(&depthStencilDesc,
                                    &m_DepthStateDisable); // 深度無効ステート
...
void Renderer::Uninit() {
  ...
  // まだ解放されていないメンバ（BlendStateやDepthStencilStateなど）もここに追加する必要があります
  if (m_DepthStateEnable)
    m_DepthStateEnable->Release();
  if (m_DepthStateDisable)
...
void Renderer::End() { m_SwapChain->Present(1, 0); }
...
void Renderer::SetWorldViewProjection2D() {
  ...
  XMMATRIX projection;
  projection = XMMatrixOrthographicOffCenterLH(0.0f, SCREEN_WIDTH,
...
void Renderer::SetProjectionMatrix(XMMATRIX ProjectionMatrix) {
  ...
  m_DeviceContext->UpdateSubresource(m_ProjectionBuffer, 0, NULL, &projectionf,
                                     0, 0);
}

void Renderer::SetMaterial(MATERIAL Material) {
  m_DeviceContext->UpdateSubresource(m_MaterialBuffer, 0, NULL, &Material, 0,
                                     0);
}
...
void Renderer::CreateVertexShader(ID3D11VertexShader **VertexShader,
  ...
  D3D11_INPUT_ELEMENT_DESC layout[] = {
      ...
      {"NORMAL", 0, DXGI_FORMAT_R32G32B32_FLOAT, 0, 4 * 3,
       D3D11_INPUT_PER_VERTEX_DATA, 0},
      {"COLOR", 0, DXGI_FORMAT_R32G32B32A32_FLOAT, 0, 4 * 6,
       D3D11_INPUT_PER_VERTEX_DATA, 0},
      {"TEXCOORD", 0, DXGI_FORMAT_R32G32_FLOAT, 0, 4 * 10,
       D3D11_INPUT_PER_VERTEX_DATA, 0}};
...
void Renderer::CreatePixelShader(ID3D11PixelShader **PixelShader,
                                 const char *FileName) {
  FILE *file;
  long int fsize;
